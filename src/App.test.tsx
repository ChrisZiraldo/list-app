...
describe('App', () => {
  it('creates a list from the phone-first capture form', async () => {
    ...
    const fetch = vi.fn()
      ...
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: 'list-1', title: 'Weekend', kind: 'todo' }), { status: 201, headers: { 'Content-Type': 'application/json' } }));
  ...
  it('captures optional priority, due date, and note when adding an item', async () => {
    ...
    const fetch = vi.fn()
      .mockResolvedValueOnce(new Response('[]', { status: 200, headers: { 'Content-Type': 'application/json' } }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: 'list-1', title: 'Weekend', kind: 'todo', pinned: false, favorite: false }), { status: 201, headers: { 'Content-Type': 'application/json' } }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: 'item-1', text: 'Pack bags', completed: false, note: 'check passport', priority: 'high', dueDate: '2026-08-01', position: 0 }), { status: 201, headers: { 'Content-Type': 'application/json' } }));
    vi.stubGlobal('fetch', fetch);
    ...
    await waitFor(() => expect(fetch).toHaveBeenLastCalledWith('/lists/api/lists/list-1/items', expect.objectContaining({
      method: 'POST',
  ...
  it('deletes a list after confirming', async () => {
    ...
    const fetch = vi.fn()
      .mockResolvedValueOnce(new Response(JSON.stringify([{ id: 'list-1', title: 'Weekend', kind: 'todo', pinned: false, favorite: false }]), { status: 200, headers: { 'Content-Type': 'application/json' } }))
      .mockResolvedValueOnce(new Response(JSON.stringify({ id: 'list-1', title: 'Weekend', kind: 'todo', pinned: false, favorite: false, items: [] }), { status: 200, headers: { 'Content-Type': 'application/json' } }))
