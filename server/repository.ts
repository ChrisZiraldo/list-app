...
export class ListsRepository {
  ...
  private createSchema() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS items (
        ...
        completed_at TEXT, priority TEXT, due_date TEXT, reminder_at TEXT, snoozed_until TEXT, note TEXT,
        created_at TEXT NOT NULL, updated_at TEXT NOT NULL,
    ...
    if (!columns.some((column) => column.name === 'reminder_at')) this.db.exec('ALTER TABLE items ADD COLUMN reminder_at TEXT');
  }
  ...
  createList(input: { title: string; kind: ListKind }): List {
    ...
    this.db
      .prepare('INSERT INTO lists (id,title,kind,pinned,favorite,created_at,updated_at) VALUES (@id,@title,@kind,@pinned,@favorite,@createdAt,@updatedAt)')
      .run({ ...list, pinned: 0, favorite: 0 });
    return list;
  }
  ...
  listLists(): List[] {
    return (
      this.db
        .prepare(
          'SELECT id,title,kind,CAST(pinned AS INTEGER) AS pinned,CAST(favorite AS INTEGER) AS favorite,created_at AS createdAt,updated_at AS updatedAt FROM lists ORDER BY created_at, id',
        )
        .all() as ListRow[]
    ).map(toList);
  }
  ...
  updateList(listId: string, input: { title?: string; kind?: ListKind; pinned?: boolean; favorite?: boolean }): List {
    ...
    this.db
      .prepare('UPDATE lists SET title = ?, kind = ?, pinned = ?, favorite = ?, updated_at = ? WHERE id = ?')
  ...
  createItem(input: { listId: string; text: string }): Item {
    ...
    const position = (
      this.db.prepare('SELECT COALESCE(MAX(position), -1) + 1 AS position FROM items WHERE list_id = ?').get(input.listId) as { position: number }
    ).position;
    const timestamp = now();
    const item: Item = {
      id: randomUUID(),
    ...
    this.db
      .prepare(
        'INSERT INTO items (id,list_id,text,position,completed,completed_at,priority,due_date,reminder_at,snoozed_until,note,created_at,updated_at) VALUES (@id,@listId,@text,@position,@completed,@completedAt,@priority,@dueDate,@reminderAt,@snoozedUntil,@note,@createdAt,@updatedAt)',
      )
      .run({ ...item, completed: 0 });
    return item;
  }
  ...
  getItems(listId: string): Item[] {
    return (
      this.db
        .prepare(
          'SELECT id,list_id AS listId,text,position,CAST(completed AS INTEGER) AS completed,completed_at AS completedAt,priority,due_date AS dueDate,reminder_at AS reminderAt,snoozed_until AS snoozedUntil,note,created_at AS createdAt,updated_at AS updatedAt FROM items WHERE list_id = ? ORDER BY position',
        )
...
function toList(row: ListRow): List {
  return { ...row, pinned: Boolean(row.pinned), favorite: Boolean(row.favorite) };
}
function toItem(row: ItemRow): Item {
  const completed = Boolean(row.completed);
  return { ...row, completed, done: completed };
}
