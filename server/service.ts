...
export class ListsService {
  ...
  constructor(database: Database.Database) {
    this.repository = new ListsRepository(database);
  }
  ...
  createItem(listId: string, input: ItemInput): Item {
    ...
    this.repository.db
      .prepare(
        'UPDATE items SET note = ?, priority = ?, due_date = ?, reminder_at = ?, snoozed_until = ?, completed = ?, completed_at = ?, updated_at = ? WHERE id = ?',
      )
      .run(
        note,
        priority,
        ...
        completed ? 1 : 0,
        completed ? (input.completedAt ?? new Date().toISOString()) : null,
  ...
  updateItem(itemId: string, input: Partial<ItemInput>): Item {
    ...
    this.repository.db
      .prepare(
        'UPDATE items SET text = ?, note = ?, priority = ?, due_date = ?, reminder_at = ?, snoozed_until = ?, completed = ?, completed_at = ?, updated_at = ? WHERE id = ?',
      )
      .run(
        ...
        completed ? 1 : 0,
        completed ? (item.completedAt ?? new Date().toISOString()) : null,
        new Date().toISOString(),
        itemId,
      );
  ...
  moveItem(itemId: string, beforeItemId?: string): void {
    this.repository.transaction(() => {
      ...
      items.splice(
        index,
        0,
        this.repository.getItems(item.listId).find((candidate) => candidate.id === itemId)!,
      );
